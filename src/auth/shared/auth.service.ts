...
export class AuthService {
    ...
    async login(user: LoginDTO) {
        ...
        const { access_token, refresh_token, first_access_token } = await this.getTokens(userSaved.user_enrollment, userSaved.user_name, userSaved.user_profile_id);
    ...
    //         ValidType.NO_SPACE,
    //         ValidType.SPECIAL_CHARACTER,
    //         ValidType.MINIMUM_ONE_NUMBER
    //     )
      
    //       Validations.getInstance().verifyLength(
    //         firstAccessDto.confirmation_password, 'Password', 6, 12
    //     )
    ...
    async refreshToken(id: number, refreshToken: string) {
        ...
        const { access_token, refresh_token } = await this.getTokens(user.user_enrollment, user.user_name, user.user_profile_id);
    ...
    async getTokens(enrollment: string, name: string, profile_id: number): Promise<Tokens> {
